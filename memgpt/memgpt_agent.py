...
from .user_agent import console
...
class AIAgent(Agent):
    ...
    def __init__(self, model, system, functions, interface, persistence_manager, persona_notes, human_notes, messages_total=None, persistence_manager_init=True, first_message_verify_mono=True, skip_verify=False):
        # gpt-4, gpt-3.5-turbo
        self.model = model
        ...
        printd(f"AIAgent initialized, self.messages_total={self.messages_total}")
    ...
    async def generate_reply(self, user_message: str = None):
        with console.status("[bold cyan]Thinking...") as status:
            while True:
                new_messages, heartbeat_request, function_failed, token_warning = await self.step(user_message, first_message=False)
        ...
        # Not really useful return value, for now
        return None
